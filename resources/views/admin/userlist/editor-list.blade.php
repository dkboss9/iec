...
@section('title', 'FSTV | Editor list')
            ...
            <div class="panel panel-default card-view">
                <div class="panel-heading">
                    ...
                    <div class="pull-right">
                        <a href="{{ route('editor.create') }}" class="btn btn-default btn-rounded" style="border-bottom: 50%">
                                        ...
                                        @if ($data)
                                            @foreach ($data as $row) 
                                                <tr>
                                                    ...
                                                    <td><ul>
                                                            <li>{{ ($row->editor_info['blog'] == 1) ? 'Blogs' : ''}}</li>
                                                            <li>{{ ($row->editor_info['gallery'] == 1) ? 'Gallery' : ''}}</li>
                                                        </ul>   
                                                    </td>
                                                    <td>
                                                        <ul class="nav nav-pills">
                                                            <li>
                                                                <a data-toggle="tooltip" data-placement="bottom" title="edit" href="{{ route('editor.edit', $row->id) }}">
                                                                    <i class="fa fa-edit fa-2x" style="color:#878787;"></i>
                                                                </a> 
                                                            </li>
                                                            <li>
                                                                <a data-toggle="tooltip" data-placement="bottom" title="delete" href="">
                                                                    {{ Form::open(['url'=>route('editor.destroy', $row->id), 'class'=>'form float-right', 'onsubmit'=>'return confirm("Are you sure to delete??")']) }}
                                                                    @method('delete')
