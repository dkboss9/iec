...
class WeekWinner extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'week:winner';
     ...
     * @var string
     */
    protected $description = 'Weekly Winner.';
    ...
    public function handle()
    {
        ...
        $win = WeeklyWinner::whereBetween('created_at', [$start_week, $end_week])->inRandomOrder()->first();

        $userid = Device::where('user_id',$win->added_by)->latest()->first();

        if ($userid) {
            $db= WeeklyWinner::create([
                'date' => $win->created_at->format("Y-m-d"),
            ...
            if($notification['device_token'] != "" ){
                ...
                $msg = array
                    (
                    'body' 	=> "You are the winner of last week's quiz.",
